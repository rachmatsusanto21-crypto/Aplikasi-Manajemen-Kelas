...
export default function GoogleSync({
  ...
  const handleLocalBackupDownload = () => {
    try {
      ...
      const encryptedData = encryptData(payloadStr);
      ...
      const jsonStr = JSON.stringify(backupWrapper, null, 2);
      ...
      const fileName = `Backup_Guru_Asisten_${dateStr}_${timeStr}.json`;
      
      const link = document.createElement('a');
      link.href = url;
      link.download = `Backup Guru Asisten/${fileName}`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      alert(`Berhasil membuat cadangan lokal!\nFile tersimpan sebagai: Backup Guru Asisten/${fileName}\n\nSilakan simpan file ini di dalam folder khusus "Backup Guru Asisten" di komputer/HP Anda.`);
    } catch (err: any) {
      alert(`Gagal mengunduh cadangan lokal: ${err.message}`);
    }
  };
  ...
  const handleLocalBackupUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!window.confirm('Apakah Anda yakin ingin memulihkan data dari file cadangan lokal ini? Data saat ini akan digantikan.')) {
      e.target.value = '';
      return;
    }
    ...
    reader.onload = (event) => {
      try {
        ...
        if (payload.data) {
          decryptedStr = decryptData(payload.data);
        } else {
          decryptedStr = text;
        }
        ...
        onRestoreDatabase(parsed);
        alert('Data berhasil dipulihkan dari file cadangan lokal dengan sukses!');
      } catch (err: any) {
        console.error('Local restore failed:', err);
        alert(`Gagal memulihkan cadangan lokal: ${err.message || 'Format file tidak sesuai'}`);
      }
    };
    reader.readAsText(file);
    e.target.value = '';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
          ...
          {/* Section: Local Drive Backup (Offline) */}
          <div className="border-t border-slate-100 dark:border-slate-800/80 pt-4">
            <div className="flex items-center space-x-1.5 mb-2.5">
              <Terminal className="w-4 h-4 text-indigo-500" />
              <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100">Pencadangan Drive Lokal (Komputer / HP)</h4>
            </div>
            <p className="text-[10px] text-slate-400 mb-3">Unduh file cadangan Anda secara lokal. File akan tersimpan ke dalam folder virtual <span className="font-semibold text-slate-600 dark:text-slate-300">"Backup Guru Asisten"</span> dengan nama file berdasarkan tanggal dan jam pengerjaan.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              <div className="border border-slate-150 dark:border-slate-800/80 p-3 rounded-lg flex flex-col justify-between bg-slate-50/30 dark:bg-slate-850/10">
                <div>
                  <h5 className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center space-x-1">
                    <FileDown className="w-3.5 h-3.5 text-indigo-500" />
                    <span>Unduh Cadangan Lokal</span>
                  </h5>
                  <p className="text-[9px] text-slate-400 mt-0.5">Ekspor seluruh database aman ke penyimpanan lokal Anda.</p>
                </div>
                <button
                  onClick={handleLocalBackupDownload}
                  className="bg-slate-700 hover:bg-slate-600 text-white text-[11px] font-bold py-1.5 rounded-lg mt-3 transition-all flex items-center justify-center space-x-1 cursor-pointer"
                >
              ...
              <div className="border border-slate-150 dark:border-slate-800/80 p-3 rounded-lg flex flex-col justify-between bg-slate-50/30 dark:bg-slate-850/10">
                <div>
                  <h5 className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center space-x-1">
                    ...
                    <span>Pulihkan dari File Lokal</span>
                  </h5>
                  <p className="text-[9px] text-slate-400 mt-0.5">Pilih file cadangan JSON lokal untuk memulihkan seluruh data.</p>
                </div>
                <label className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[11px] font-bold py-1.5 rounded-lg mt-3 transition-all flex items-center justify-center space-x-1 cursor-pointer border border-dashed border-slate-300 dark:border-slate-700 text-center">
